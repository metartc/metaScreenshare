...
function SrsRtcPlayerAsync() {	
    ...
    self.playSrs = async function(url) {
    ...
    self.playZlm = async function(url) {
        var conf = self.__internal.prepareUrl(url);
        var zurl=conf.urlObject;
       // console.log("conf.apiUrl: ", conf.apiUrl);
        var zport=zurl.port;
        if(zport==1985) zport=80;
        var zlmUrl="http://"+zurl.server+":"+zport+"/index/api/webrtc?app="+zurl.app+"&stream="+zurl.stream+"&type=play";
        //console.log("url:"+zlmUrl);
        ...
        var session = await new Promise(function(resolve, reject) {
            ...
            $.ajax({
                type: "POST", url: zlmUrl, data: offer.sdp,
