...
void YangYuvConvert::loadLib() {
	...
	yang_ABGRToI420=(int (*)(const uint8_t* src_argb, int src_stride_argb,
	               uint8_t* dst_y, int dst_stride_y, uint8_t* dst_u,int dst_stride_u,
	               uint8_t* dst_v,  int dst_stride_v, int width,int height)) m_lib.loadFunction("ABGRToI420");
...
{
    memcpy(dst,src,width*height);//y分量
    memcpy(dst+width*height,src+width*height+width*height/4,width*height/4);//V分量
    memcpy(dst+width*height+width*height/4,src+width*height,width*height/4);//u分量
    return 0;
}
...
int32_t YangYuvConvert::nv21tonv12(uint8_t* src,uint8_t *dst,int32_t width,int32_t height){
	return yang_NV21ToNV12((const uint8_t*) src, width ,src+width*height,width,
...
int32_t YangYuvConvert::bgratonv12(uint8_t* src_bgra,uint8_t *dst,int32_t width,int32_t height){
	return yang_ABGRToNV12((const uint8_t*) src_bgra, width * 4, dst,
				width, dst + (width * height), width ,
				width, height);
}


int32_t YangYuvConvert::bgratoI4202(uint8_t* src_bgra,uint8_t *dst,int32_t width,int32_t height,int32_t stride) {
	//return yang_BGRAToI420((const uint8_t*) src_bgra, width * 4, dst,

	return yang_ABGRToI420((const uint8_t*) src_bgra, stride, dst,
			width, dst + (width * height), (width >> 1),
			dst + (width * height) + (int) (width * height / 4), (width >> 1),
			width, height);
}
